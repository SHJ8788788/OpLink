...
namespace OpcClient
{
    ...
    public static class TagConfig
    {
        ...
        /// <param name="blockName"></param>
        /// <returns></returns>
        public static bool EditBlock(string groupName,string blockName,string newBlockName)
        {
            try
            {
                XElement xGroups = XElement.Load(Path);
                XElement xGroup = xGroups.Elements("Group").Where(p => p.Attribute("Name").Value == groupName).FirstOrDefault();
                XElement xBlock = xGroup.Elements("Block").Where(p => p.Attribute("Name").Value == blockName).FirstOrDefault();
                xBlock.SetAttributeValue("Name", newBlockName);
                xGroups.Save(Path);
                return true;
            }
            catch (Exception)
        ...
        {
            var config = ConfigurationManagerExtend.SectionsCast<TagConfiguration>("Tag").FirstOrDefault().Value;
            return config.TagPath;
        }
        #endregion
        /// <summary>
        /// 查询xml配置文件中的tags
